...
const offer = getOfferBySlug('abonnement-iptv-1-an')!;
...
export default function Abonnement1AnPage() {
    ...
    <>
      ...
      <section className="py-12 sm:py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
          {offer.badge && (
            <span className="mb-4 inline-block rounded-full bg-rouge px-4 py-1.5 text-sm font-bold text-white">
              🔥 {offer.badge}
            </span>
          )}
          ...
          <div className="mb-8 flex flex-col items-center gap-3">
            ...
            {offer.savingsNote && (
              <span className="inline-block rounded-full bg-vert/10 px-4 py-1.5 text-sm font-semibold text-vert">
                {offer.savingsNote}
              </span>
            )}
          </div>
      ...
      {/* FAQ */}
      <FAQSection faqs={productFAQ} title="Questions fréquentes sur l'abonnement annuel" />

      {/* Bottom CTA */}
      <section className="border-t border-border py-12">
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          <h2 className="mb-3 text-xl font-bold text-foreground sm:text-2xl">
            Le meilleur prix pour 12 mois
          </h2>
          <p className="mb-6 text-sm text-muted">
            {offer.savingsNote} — Paiement par Wave ou Orange Money.
          </p>
          <WhatsAppCTA
            offerName={offer.name}
            price={offer.priceLabel}
